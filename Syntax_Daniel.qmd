...
# Column 1
m1 <- feols(
  ln_emp_pop ~ post | fips + year + month,
  cluster = ~fips, data = df_sub
)

# Column 2
m2 <- feols(
  ln_emp_pop ~ post + successful | fips + year + month,
  cluster = ~fips, data = df_sub
)

# Column 3
m3 <- feols(
  ln_emp_pop ~ successful + non_us_t + int_l
  | fips + year + month^year,
  cluster = ~fips, data = df_sub
)
...
# Panel B
m1_B <- feols(ln_real_qp1_pop ~ post |
                fips + year + month, cluster = ~fips, data = df_sub)
m2_B <- feols(ln_real_qp1_pop ~ post + successful |
                fips + year + month, cluster = ~fips, data = df_sub)
m3_B <- feols(ln_real_qp1_pop ~ post + successful + non_us_t + int_l |
                fips + year + month^year, cluster = ~fips, data = df_sub)
m4_B <- feols(ln_real_qp1_pop ~ post + successful + non_us_t + int_l +
                aa_assass + aa_armed + aa_bomb + aa_facility +
                ww_firearm + ww_explo + ww_incend |
                fips + year + month^year, cluster = ~fips, data = df_sub)
m5_B <- feols(ln_real_qp1_pop ~ post + successful + non_us_t + int_l |
                fips + year + div_9_all^year, cluster = ~fips, data = df_sub)
m6_B <- feols(ln_real_qp1_pop ~ post + successful + non_us_t + int_l +
                aa_assass + aa_armed + aa_bomb + aa_facility +
                ww_firearm + ww_explo + ww_incend |
                fips + year + div_9_all^year, cluster = ~fips, data = df_sub)

# Panel C
m1_C <- feols(ln_real_qp1_job ~ post |
                fips + year + month, cluster = ~fips, data = df_sub)
m2_C <- feols(ln_real_qp1_job ~ post + successful |
                fips + year + month, cluster = ~fips, data = df_sub)
m3_C <- feols(ln_real_qp1_job ~ post + successful + non_us_t + int_l |
                fips + year + month^year, cluster = ~fips, data = df_sub)
m4_C <- feols(ln_real_qp1_job ~ post + successful + non_us_t + int_l +
                aa_assass + aa_armed + aa_bomb + aa_facility +
                ww_firearm + ww_explo + ww_incend |
                fips + year + month^year, cluster = ~fips, data = df_sub)
m5_C <- feols(ln_real_qp1_job ~ post + successful + non_us_t + int_l |
                fips + year + div_9_all^year, cluster = ~fips, data = df_sub)
m6_C <- feols(ln_real_qp1_job ~ post + successful + non_us_t + int_l +
                aa_assass + aa_armed + aa_bomb + aa_facility +
                ww_firearm + ww_explo + ww_incend |
                fips + year + div_9_all^year, cluster = ~fips, data = df_sub)
...
dict_common <- c(
  ...
  meventperyear = "Number of attacks",
