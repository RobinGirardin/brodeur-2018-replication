...
Before going for the analysis, some preprocessing needs to be done, as predictor employement rate of non-attacked countries using 
the `SUCESS`, `POST` and fixed-effects values from the attacked counties posed some issues.
We'll consider neighboring counties counties of the same state.
...
Now, let's see wether we get a significant interaction result by analysis the whole state of Arizone (01) for every counties.
There is no significant interaction effect between the SUCESS treatment variable for every pair of targetted-neighbor counties of Arizona, except for county 1055.
...
ggplot(data = df %>% drop_na(), 
       mapping = aes(y=ln_emp_pop,x=year, color=factor(successful)))+
